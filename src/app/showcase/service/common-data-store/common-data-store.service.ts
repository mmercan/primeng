...
export class CommonDataStoreService<T> {
    ...
    getAll() {
        ...
        this.superhttp.get(this.baseUrl).map(response => response.json()).subscribe(data => {
            this.dataStore.dataset = data.data;
    ...
    get(id: number | string) {
        this.superhttp.get(`${this.baseUrl}/${id}`).map(response => response.json()).subscribe(data => {
            let notFound = true;
